...
All constraints expressed as plain dicts (the format used by chain JSONL files)
rather than dataclass instances — the shuffler handles both formats.

Copied from v2 tests/test_shuffler.py with source label updated to game domain.
"""
...
def _make_chain(chain_id: str = "chess_standard_00001", n: int = 5) -> dict:
...
class TestChainMetadata:
    ...
    def test_chain_id_updated_with_seed(self):
        ...
        assert shuffled["chain_id"] == "chess_standard_00001_shuffled_42"

    def test_chain_id_uses_given_seed(self):
        chain = _make_chain()
        s1 = shuffle_chain(chain, seed=1337)
        assert s1["chain_id"] == "chess_standard_00001_shuffled_1337"

    def test_match_id_preserved(self):
        chain = _make_chain()
        shuffled = shuffle_chain(chain, seed=42)
        assert shuffled["match_id"] == "game_001"

    def test_source_preserved(self):
        chain = _make_chain()
        shuffled = shuffle_chain(chain, seed=42)
        assert shuffled.get("source") == "chess_standard"
